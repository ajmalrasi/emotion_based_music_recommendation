...
class RetinaFace(object):
    ...
    def __init__(self, jit_file = "./detector/models/resnet18.zip", cpu = False, top = 1500, conf = 0.1):
    ...
    def align_and_crop(self, input_image, dets, resolution=160, flip = False):
        ...
        for detection in dets:
            ...
            if flip:
                ...
                flip_crop = np.expand_dims(flip_crop/255., axis=0).astype(np.float32)
                crops_flips = np.concatenate((crops_flips, flip_crop), axis=0)
                
            crop = np.expand_dims(crop/255., axis=0).astype(np.float32)
...
if __name__ == "__main__":
    ...
    face = RetinaFace("./detector/models/mobilenet0.25.zip", False, 10)
    ...
    while(cap.isOpened()):
        ...
        if len(detections) > 0:
            x1, y1, x2, y2 = tuple([int(x) for x in detections[0][:4]])
